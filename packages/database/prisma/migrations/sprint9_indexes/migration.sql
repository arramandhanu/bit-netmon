...
CREATE INDEX IF NOT EXISTS idx_devices_polling ON devices (polling_enabled) WHERE polling_enabled = true;

-- ─── Alert History ──────────────────────────────────────
-- Dashboard query: active alerts sorted by time
CREATE INDEX IF NOT EXISTS idx_alert_history_state ON alert_history (state) WHERE state = 'triggered';
