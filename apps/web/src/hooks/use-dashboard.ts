...
export function useDashboard(refreshInterval = 30000) {
  ...
  const fetchDashboard = useCallback(async () => {
    try {
      ...
      const totalDevices =
        devicesRes.data?.total ||
        devicesRes.data?.data?.length ||
        metrics.length;

      const devicesUp = metrics.filter(
        (m: DeviceMetricSnapshot) => m.device_status === "up",
      ).length;
      const devicesDown = metrics.filter(
        (m: DeviceMetricSnapshot) => m.device_status === "down",
      ).length;
      const devicesWarning = metrics.filter((m: DeviceMetricSnapshot) => {
      ...
      const cpuValues = metrics
        .filter((m: DeviceMetricSnapshot) => m.cpu_utilization != null)
        .map((m: DeviceMetricSnapshot) => m.cpu_utilization!);
      const memValues = metrics
        .filter((m: DeviceMetricSnapshot) => m.memory_percent != null)
        .map((m: DeviceMetricSnapshot) => m.memory_percent!);
      ...
      const topCpuDevices = [...metrics]
        .filter((m: DeviceMetricSnapshot) => m.cpu_utilization != null)
        .sort(
          (a: DeviceMetricSnapshot, b: DeviceMetricSnapshot) =>
            (b.cpu_utilization || 0) - (a.cpu_utilization || 0),
        )
        .slice(0, 5);
